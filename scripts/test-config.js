...
async function testYouTubeAPI() {
  ...
  try {
    ...
    if (response.ok && data.items) {
      ...
      logError(
        `Erro na YouTube API: ${data.error?.message || "Unknown error"}`,
      );
      return false;
    }
  } catch (error) {
...
async function testOpenAI() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      logError(
        `Erro na OpenAI API: ${error.error?.message || "Unknown error"}`,
      );
      return false;
    }
  } catch (error) {
...
async function testAnthropic() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const error = await response.json();
      logError(
        `Erro na Anthropic API: ${error.error?.message || "Unknown error"}`,
      );
      return false;
    }
  } catch (error) {
