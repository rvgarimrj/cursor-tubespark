...
export async function getUserYouTubeChannel(userId: string): Promise<YouTubeChannel | null> {
  const supabase = createClient();
...
export async function saveYouTubeChannel(channel: Omit<YouTubeChannel, 'created_at' | 'updated_at'>): Promise<YouTubeChannel | null> {
  const supabase = createClient();
